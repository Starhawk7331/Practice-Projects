...
class FRUIT:
    ...
    def change_location(self):
        self.y = random.randint(0, cell_number - 1) * cell_number
        self.x = random.randint(0, cell_number - 1) * cell_number
        self.pos = Vector2(self.x, self.y)
        global fruit_location
        fruit_location = self.pos
...
class snake:
    def __init__(self):
        self.body = [Vector2(2 * cell_size, 0), Vector2(cell_size, 0), Vector2(0, 0)]
        self.direction = Vector2(cell_size, 0)
    ...
    def move(self, key_pressed):
        if key_pressed[pygame.K_w]:
            self.direction = Vector2(0, -1 * cell_size)
        elif key_pressed[pygame.K_s]:
            self.direction = Vector2(0, cell_size)
...
def end_game(body):
    global run
    score = len(body) - 3
    with open("C:\SEW\Python\Games\Snake_Highscore.txt", "r") as f:
        highscore = int(f.readline())
    if score > highscore:
        highscore = score
        with open("C:\SEW\Python\Games\Snake_Highscore.txt", "w") as f:
